...
class MenuData(BaseModel):
    ...
    nutrition_per_100g: Optional[Dict[str, float]] = Field(
        None, 
        description="КБЖУ на 100г"
    )
    ...
    source: Optional[str] = Field(
        None,
        description="Источник распознавания: chatgpt_vision, gemini_vision, ocr"
    )
    raw_text: Optional[str] = Field(None, description="Сырой текст из OCR")
    nutrition_not_found: Optional[bool] = Field(
        False,
        description="Флаг что КБЖУ не найдено"
    )
...
class PhotoStateData(BaseModel):
    ...
    menu_data: Optional[MenuData] = Field(
        None,
        description="КРИТИЧНО: Распознанное КБЖУ - НЕ ТЕРЯТЬ при пересоздании состояния!"
    )
