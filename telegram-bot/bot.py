...
async def main():
    ...
    # + дашборд) и/или polling. В проде — только сервер (webhook). На dev — сервер
    # И polling одновременно (force_polling): сервер нужен для /health, дашборда и
    # agent-tools, а апдейты Telegram тянутся polling'ом без публичного TLS.
    try:
        tasks = []
        if webhook_enabled:
