...
# «120/80». Всё сообщение целиком (anchored) — одно значение: либо число с опц.
# единицей (вес/доза), либо АД «XXX/YY» (+пульс). Одиночные числа ограничены 3
# цифрами, чтобы не ловить годы («2024»).
_SHORT_VALUE_RE = re.compile(
    r"^\s*(?:"
    r"\d{1,3}(?:[.,]\d{1,2})?\s*(?:кг|kg|г|g|мг|mg|мл|ml|ме|iu|шт|таб\.?|капс\.?)?"
    r"|\d{2,3}\s*[/\\]\s*\d{2,3}(?:\s*[/\\]?\s*\d{2,3})?"
    r")\s*$",
    re.IGNORECASE,
)
