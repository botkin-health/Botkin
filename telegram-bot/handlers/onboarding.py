...
async def _finish_onboarding(user, db, chat_id: int) -> None:
    ...
    await send_message(
        ...
        "⚠️ Важно: после этого пришлю карточку с распознанными калориями и БЖУ — "
        "её нужно подтвердить кнопкой «✅ Сохранить», иначе запись никуда не попадёт. "
        "Просто отправить фото или текст — недостаточно, это ещё черновик.\n\n"
        "Чтобы советы были точнее (по желанию):\n"
        "📂 просто пришли фото или PDF анализа или заключения врача — сам разберу и предложу сохранить, "
        "команда не нужна · ⌚ есть часы/тонометр — напиши какие, подскажу как подключить · 💊 добавки\n"
