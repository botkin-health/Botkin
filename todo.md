...
- [ ] **Полный переезд legacy-домена `health.orangegate.cc` → `botkin.health`.** Доменная часть бренд-миграции. Делается ОТДЕЛЬНО от рефактора имён инфры выше — nginx vhost уже разнесён и оба домена работают параллельно, остался cleanup ссылок. *Добавлено 24.05.2026.*

  **Что уже сделано (24.05.2026):**
  - ✅ Nginx vhost `botkin.health` расширен: проксирует `/admin/`, `/apple_health`, `/apple_health_v2`, `/agent/`, `/mc/`, `/healthz`, `/docs`, `/openapi.json` на `127.0.0.1:8081` через snippet `/etc/nginx/snippets/botkin-backend.conf`
  - ✅ `ADMIN_PASSWORD` (+ `ADMIN_USERNAME`) добавлен в `/opt/healthvault/.env` на сервере, контейнер `healthvault_bot` пересоздан — `/admin/` работает (раньше отдавал «Admin not configured»). Бэкап `.env.bak.1779612026`
  - ✅ 1Password обновлён: «Botkin Admin Dashboard» URL → botkin.health, «Botkin (ex-HealthVault) — service secrets» получила поля доменов/инфры, создана новая карточка «Botkin — User HAE tokens» со всеми токенами 5 пользователей
  - ✅ Старый домен `health.orangegate.cc` оставлен живым как fallback — HAE на iPhone Alex/Andrey продолжает работать без изменений
  - ✅ Бэкапы конфигов на сервере: `/etc/nginx/sites-available/botkin.health.bak.1779612107`, `/opt/healthvault/.env.production.bak.1779611943`

  **Что осталось (нужно сделать для единообразия):**
  1. **HAE на iPhone Alex и Andrey** — обновить URL в Health Auto Export автоматизации: `health.orangegate.cc/apple_health_v2` → `botkin.health/apple_health_v2`. Bearer-токен не меняется, остальные настройки не меняются. ~30 сек на каждого. Делать после того как Ника подтвердит что её сетап (она сразу с botkin.health) шлёт данные корректно.
  2. **Code/docs sweep** — заменить `health.orangegate.cc` → `botkin.health` в репозитории. На 24.05.2026 ~20 файлов: `telegram-bot/bot.py`, `telegram-bot/webhook/apple_health.py`, `telegram-bot/handlers/{commands,onboarding}.py`, `docs/landing/README.md`, `docs/research/2026-05-22_android-health-connect-export-webhook.md`, `docs/projects/2026-04_share-dashboard/{SPEC,STATUS,PLAN}.md`, `docs/user_guide/ru/{architecture,dashboard,apple-health,mcp-claude-desktop}.md`, `docs/projects/2026-04_telegram-mini-app-settings/PLAN.md`, `docs/projects/2026-04_diary-ux-redesign/PLAN.md`, `docs/ai_context/{AI_CHANGELOG,04_workflows}.md`, `docs/superpowers/specs/2026-05-12-cohort-agents-v2-design.md`, `CLAUDE.md`, `todo.md` (строка 249 — Telegram Mini App webapp URL). Один коммит find-replace + smoke-проверка.
  3. **Через 3 месяца после code sweep** (тогда логи покажут что HAE Alex/Andrey уже переключены и в трафике на legacy-домене только робот-сканеры) — отключить nginx vhost `health.orangegate.cc`, удалить DNS-запись на Cloudflare. Домен на год до экспирации можно сохранить — стоит копейки, защита от случайностей.

  **Метрика готовности к шагу 3:** `tail -F /var/log/nginx/access.log | grep health.orangegate.cc` за неделю показывает только сканеры/боты, ни одного запроса от реального HAE-клиента.
...
- [ ] **`/export` команда в боте — выгрузка своих данных одним архивом.** Несрочно (никто не экспортирует пока). Когда дойдут руки: команда в боте → ZIP с `kb_<id>.json` + CSV всех таблиц (`weights.csv`, `nutrition.csv`, `supplements.csv`, `blood_pressure.csv`, `activity.csv`) + readme. Telegram `sendDocument`. Полезно: (1) показать пользователю что у нас на него хранится, (2) экспортировать в другую нейронку, (3) AGPL-promise про «данные пользователя — его собственные». Также может быть кнопка в мини-аппе. *Добавлено 22.05.2026.*
...
- [ ] **Health Goals UI — страница «Мои цели» в мини-аппе + iOS/Android + текст-командой боту** (24.05.2026). Сейчас `health_goals` в KB (lipids/vitamin_d/metabolic с целями, протоколом-rungs, контрольными датами) **редактируется только вручную через Python**, а **видеть** их можно только спросив бота в чате. Это пропуск:
  - Mini-app не показывает текущие цели и их статус
  - Невозможно поменять цель через UI (например пользователь захотел поднять планку ЛПНП с <2.5 на <2.0)
  - Нет наглядного «прогресс к цели за последние N месяцев»

  **Что сделать:**
  1. **Новая страница в мини-аппе «Мои цели»** — карточки по системам (липиды/витD/метаболика/сон/давление). На каждой карточке: текущее значение, цель, статус (зелёный/жёлтый/красный), контрольная дата, ссылка «развернуть протокол» → показывает rungs.
  2. **CRUD endpoints в `/api/health_goals`** — GET (список), GET по ключу (с rungs), PATCH (обновить цель), POST (добавить новый rung). Helper в `core/health/goals.py`.
  3. **Тот же endpoint доступен агенту через tool** — `update_health_goal_target(system, marker, new_target)`. Тогда «измени мою цель по ЛПНП на 2.0» текстом боту → атомарная операция.
  4. **iOS/Android приложение в будущем** — то же `/api/health_goals` endpoint.

  Архитектурный pattern: **один helper → 4 клиента** (mini-app, bot-agent, native app, ручное редактирование). Тот же подход что для supplement plan (см. TODO выше).

  Также: сюда же интегрировать **визуализацию прогресса** — график «было/стало/цель» для каждого маркера. Сейчас бот рисует такие графики через `render_chart` tool, но они одноразовые. На странице целей — постоянные, обновляются при каждой новой сдаче.

- [ ] **Цели биомаркеров: hardcoded → читать из `health_goals` в KB** (24.05.2026). Сейчас цели липидов/витD/HOMA-IR живут **в двух местах**: (1) хардкод в `telegram-bot/dashboard_generator.py` (для отрисовки Attia/LE8/SCORE2 вкладок), (2) `health_goals.lipids/vitamin_d/metabolic` в `kb_<id>.json` (для агента через `/kb_value`). При обновлении планок приходится править оба места и можно забыть одно. Прецедент 24.05.2026: добавили `borderline_attia` статус в KB, но дашборд продолжил показывать «⚠ внимание, цель <1.5» из хардкода — они случайно совпали, могли бы и не совпасть. **Что сделать:** в `dashboard_generator.py` функция строящая Attia/LE8 панели должна делать `kb_value(key="health_goals.lipids.targets.ApoB_g_l.target")` вместо хардкода `<0.9`. **Один источник истины — KB.** Бонус: при добавлении нового юзера достаточно положить `kb_<id>.json` с health_goals — дашборд автоматически использует его цели.

- [ ] **Атомарные операции с supplement plan — общий helper для бота / мини-аппа / iOS/Android** (24.05.2026). Сейчас редактирование плана БАДов (что и когда принимаешь) идёт через destructive `/update_user_settings supplements=<вся_простыня>` — клиент должен прочитать список, изменить локально, отправить обратно целиком. Для мини-аппа это OK, для LLM-агента **опасно** — он может потерять одну из 11 строк при копировании. **Идея:** atomic-операции в `core/health/supplements.py`:
  ```
  add_to_plan(user_id, name, dose, slot)
  remove_from_plan(user_id, name, slot)
  update_in_plan(user_id, name, new_dose=None, new_slot=None)
  reorder_plan(user_id, new_order)
  ```
  **Три клиента — один helper:**
  - Mini-app: `POST /api/supplements/plan` (action + parameters)
  - Bot agent: tools `add_supplement_to_plan`, `remove_supplement_from_plan`, `update_supplement_in_plan`, `move_supplement_slot` — каждый дёргает соответствующий helper
  - iOS/Android: тот же `/api/supplements/plan`

  Плюсы: LLM управляет **одной строкой** а не списком (намного меньше шанс ошибки), аудит per-action, один контракт на все клиенты. Параллельно — LLM-router распознаёт паттерны «добавь второй приём X вечером», «убери Y из утра», «перенеси Z на вечер» и вызывает нужный tool.

  **Прецедент 24.05.2026:** пользователь спросил «можно ли текстом боту сказать "внеси второй приём витамина D вечером"» — формально да (текущий `update_user_settings` это сделает), но с риском что LLM потеряет элементы. Атомарные операции уберут этот риск.

- [ ] **Команда сброса контекста агента — `/agent_reset` + кнопка в мини-аппе + iOS/Android** (24.05.2026). BotkinClaw держит историю разговора в `agent_conversations` (PostgreSQL) и при каждом новом сообщении подгружает её в Claude как контекст. Если в истории есть «токсичный» ответ (например бот когда-то ответил «нет данных за май», а потом мы данные загрузили) — LLM продолжает повторять старый ответ даже когда tool отдаёт правильные данные. Сейчас лечится только ручным `DELETE FROM agent_conversations WHERE user_id=… AND created_at > …` через psql. Прецедент: 24.05.2026 — после загрузки CMD-панели бот трижды повторял «последний анализ 19 марта», пришлось чистить вручную. **Что сделать:**
  1. **Бот:** новая команда `/agent_reset` — удаляет историю агента **только** за последние 24ч (опционально с подтверждением `/agent_reset confirm`). Старше 24ч оставляет — это «долговременная память» (контекст истории здоровья). Реализовать в `handlers/` рядом с `/sync`. Использовать существующий RLS, чистить с `WHERE user_id=current_user_id`. Бэкап перед DELETE — необязателен (это сообщения, не данные).
  2. **Мини-апп:** кнопка «🔄 Сбросить контекст AI» в настройках или внизу диалога. Вызывает тот же endpoint через `/api/agent/reset` (новый endpoint в `webhook/agent_tools_api.py`, JWT-auth, ту же RLS-логику).
  3. **iOS/Android (будущее приложение):** тот же endpoint, кнопка в settings или жест свайпа на диалоге. Архитектурно — endpoint должен быть универсальным от старта, чтобы все 3 клиента (бот / мини-апп / native app) использовали одну ручку.

  Также подумать: автоматический сброс при обнаружении расхождения (если tool вернул свежие данные но в недавней истории есть «нет данных» — выкинуть оттуда tool_result+ответ автоматически). Это сложнее, отдельный спринт.

- [ ] **router_food галлюцинирует доп. блюда при follow-up корректировках** (22.05.2026): когда пользователь дописывает «забыл упомянуть, в обоих случаях запивал чаем с лимоном и сахаром» как уточнение к предыдущему meal'у, парсер не понимает, что речь о добавлении напитка к уже залогированным приёмам пищи, и сочиняет новый meal из несуществующих блюд. **Реальный кейс — Игорь @IgorLysk (telegram_id 830908046), 21.05.2026 23:08 МСК:** к двум залогированным meal'ам (пицца+нектарин+мандарин и пита+тунец+огурец) пришло follow-up «запивал стаканом чёрного чая с лимоном и сахаром в обоих случаях». Бот записал отдельный meal в 23:08: чай 400г (48 ккал) **+ выдуманный «Пирожок с сыром и маринованный огурец · 282 ккал · Б9 Ж10 У38»**, которого в сообщении не было. Итог 273 ккал, из которых 225 — галлюцинация. Что починить: (1) при `router_food` с явным маркером уточнения (`«забыл»`, `«+ ещё»`, `«в обоих случаях»`, `«к завтраку добавь»`) — не создавать новый meal, а апдейтить items предыдущих ближайших meal'ов того же user_id; (2) если LLM всё-таки решил создать новый meal — запретить «дописывать» блюда, которых нет в тексте сообщения (валидация: каждое блюдо в `items` должно иметь lexical match в исходном тексте). Воспроизводимо через `agent_conversations` где `source='router_food'`.
