...
- [ ] **🚀 Полноценный мультиюзер NutriLogBot (эти выходные, 25–26 апр)**: вместе с пользователем. Цель — открыть бот для Андрея Походни, его жены, друзей и любых желающих (путь 1 — хостинг на моём сервере, как сейчас работает жена user_id=485132). Минимально: (1) регистрация через `/start` с онбордингом (имя, возраст, пол, рост, вес, цель — дефицит/поддержание/профицит), (2) подтверждение админом или открытая регистрация — решить, (3) админка `/users` для user_id=895655 (список пользователей, блокировка), (4) проверка изоляции user_id во ВСЕХ запросах к `nutrition_log`, `supplements_log`, `activity_log`, `user_settings` (сейчас местами хардкод 895655 — аудит обязателен), (5) дефолтные настройки для новых (calorie_goal_pct=-15, FIBER_GOAL_G=30, таймзона Europe/Moscow), (6) мини-приложение должно корректно работать для любого user_id (не только 895655 — проверить bmr_avg, activity_avg, target для юзеров без Garmin-данных). Смежное: taxonomy «публичные vs личные данные» для Гармина/Apple Health — чужие пользователи свои цепляют сами или используют ручной ввод. Сначала — минимальная регистрация + аудит, остальное итеративно.
