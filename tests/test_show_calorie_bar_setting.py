...
class TestUserSettingsShowBar:
    ...
    def test_upsert_sets_show_bar_false(self):
        ...
        existing = UserSettings(user_id=485132, show_calorie_budget_bar=True)
        db = MagicMock()
        db.query.return_value.filter.return_value.first.return_value = existing

        upsert_user_settings(db, user_id=485132, show_calorie_budget_bar=False)
    ...
    def test_show_bar_false_reads_from_settings(self):
        ...
        mock_settings = UserSettings(user_id=485132, show_calorie_budget_bar=False)
        db = MagicMock()
        db.query.return_value.filter.return_value.first.return_value = mock_settings

        _us = get_user_settings(db, user_id=485132)
