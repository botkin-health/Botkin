...
def test_user_without_container_returns_ok(MockSession):
    ...
    MockSession.return_value = db
    user = MagicMock(
        telegram_id=REDACTED_ID, container_id=None, container_port=None, is_active=True, onboarding_step="done"
    )
    db.query.return_value.filter_by.return_value.first.return_value = user

    r = client.post("/telegram/webhook", json=_tg_payload(from_id=REDACTED_ID, text="привет"))
    assert r.status_code == 200
