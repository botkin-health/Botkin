...
class TestMenuDataPreservation:
    ...
    def test_menu_data_preserved_when_state_recreated(self, state_manager, sample_menu_data):
        """
        ...
        assert new_state.data["menu_data"] is not None, "КРИТИЧЕСКИЙ БАГ: menu_data потерян при пересоздании состояния!"

        assert new_state.data["menu_data"]["dish_name"] == "Рыбные кебабы с гарниром из зелёного горошка"
    ...
    def test_menu_data_updated_if_new_provided(self, state_manager, sample_menu_data):
        ...
        existing_state = UserState(
            user_id=user_id, state="waiting_description", data={"photo_paths": ["/tmp/old.jpg"], "menu_data": old_menu}
        )
    ...
    def test_update_menu_data_preserves_other_fields(self, state_manager, sample_menu_data):
    ...
    def test_preserve_and_merge_keeps_critical_keys(self):
        ...
        existing = {"menu_data": {"calories": 500}, "caption": "старая подпись", "photo_paths": ["/old/path"]}

        new = {"caption": "новая подпись", "photo_paths": ["/new/path"]}

        merged = preserve_and_merge_state_data(existing, new, preserve_keys=["menu_data"])
...
class TestStateHelpers:
    ...
    def test_get_menu_data_returns_none_for_missing_state(self, state_manager):
        """Проверить что get_menu_data возвращает None если нет состояния."""
        menu = get_menu_data_from_state("nonexistent_user")
        assert menu is None

    def test_get_menu_data_returns_typed_object(self, state_manager, sample_menu_data):
        """Проверить что get_menu_data возвращает MenuData объект."""
        user_id = "test"
        state = UserState(user_id=user_id, state="waiting_description", data={"menu_data": sample_menu_data})
