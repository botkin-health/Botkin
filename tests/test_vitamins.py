...
class TestVitaminLogging:
    ...
    def test_vitamin_logging_basic(self, test_db):
        ...
        logs = (
            test_db.query(SupplementLog)
            .filter(SupplementLog.user_id == user_id, SupplementLog.supplement_name == "Магний")
            .all()
        )
    ...
    def test_vitamin_variations(self, test_db):
        ...
        logs = test_db.query(SupplementLog).filter(SupplementLog.user_id == user_id).all()
    ...
    def test_vitamin_yesterday_logging(self, test_db):
        ...
        create_supplement_log(
            test_db, user_id=user_id, date=yesterday, time=datetime.now().time(), supplement_name="Цинк", dosage=None
        )

        # Проверяем дату
        log = (
            test_db.query(SupplementLog)
            .filter(SupplementLog.user_id == user_id, SupplementLog.supplement_name == "Цинк")
