...
> ночного синка: путь `data/derived/<id>/workouts_log.json` содержит id, а агент и
> дашборд ищут строго по нему. Без переменной сборка тренировок падает с явной ошибкой
> (issue #474; до фикса она молча писала `workouts_log_0.json`, и месяц агент отвечал
> из обеднённого DB-фолбэка без пульса и зон).

## Производные per-user файлы переживают деплой

Тренировки с зонами, сводка по воздуху и снимок биомаркеров собираются ночным
синком в `/app/data/derived/<telegram_id>/{workouts_log,env_data,biomarkers}.json`
— то есть на bind-mount `/opt/botkin/data`, а не внутри образа. Единственный
резолвер пути — `core/infra/derived_paths.py` (override `BOTKIN_DERIVED_DIR`).

Так сделано после #480: раньше файлы лежали в `/app/telegram-bot/` и **стирались
при каждом пересоздании контейнера**. До ближайшего прогона cron (каждые 30 мин,
но только в окне 04–20 UTC) агент и дашборд молча деградировали к обеднённым
источникам — без пульса, зон и training load. Вечерний деплой оставлял дыру до утра.

Чтение умеет фолбэк на старое место, поэтому первый выкат не создаёт провала;
после него файлы пишутся уже только в новую раскладку, атомарно (tmp + `os.replace`).

Каталог должен принадлежать uid 10001 (`botkin`) — иначе сборка упадёт с подсказкой.
Разово, после первого выката с этой раскладкой:
