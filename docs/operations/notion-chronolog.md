...
- `API-patch-block-children` без параметров дописывает в **конец**. С параметром `after: <id блока>` вставляет сразу после этого блока — так можно добавить запись под сегодняшним заголовком, не читая страницу: `API-get-block-children` с `page_size: 3` отдаёт id верхнего заголовка. Вставить *перед* самым первым блоком так нельзя — для новой даты остаётся `update_content`. Ответ `patch-block-children` тоже большой (~110 тыс. символов), но уходит в файл, а не в контекст.
