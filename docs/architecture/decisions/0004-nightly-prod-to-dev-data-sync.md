...
**Плюсы:** дев получает реальные данные, сохраняя свой тест в natural-key таблицах; прод недостижим на запись; идемпотентно (повторный прогон не плодит дубли); устойчиво к включению RLS.
