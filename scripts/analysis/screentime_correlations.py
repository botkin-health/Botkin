...
if "Большевик" in netatmo_data:
    for ts_str, metrics in netatmo_data["Большевик"].items():
        ...
        if len(metrics) > 1 and metrics[1] is not None:
            if date_str not in co2_by_date:
                co2_by_date[date_str] = []
...
for r in results:
    co2 = r["CO2"]
    print(
        f"{r['Date']} |     {r['SleepScore']:2}      |  {r['DeepSleep_pct']:4.1f}  |   {r['Stress']:2}   |   {r['Pickups']:2}    |   {co2:>5}   |  {r['Msg_hr']:4.1f} |   {r['PreBedST_hr']:4.2f}    |  {r['TotalST_hr']:4.1f}"
    )
