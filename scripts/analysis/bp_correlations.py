    ...
    for entry in entries:
        ...
        for meal in meals:
            for item in meal.get("items", []):
                ...
                # Check alcohol
                if any(
                    x in food for x in ["вино", "пиво", "ром", "водка", "виски", "коньяк", "alcohol", "beer", "wine"]
                ):
                ...
                if any(
                    x in food
                    for x in ["соль", "соевый", "соус", "чипсы", "колбаса", "бекон", "холодец", "рыба соленая"]
                ):
...
def analyze():
    ...
    for d in sorted_dates:
        ...
        print(
            f"{d:<12} | {b['sys']:>3.0f}/{b['dia']:<3.0f}      | {s:>4.1f}h | {w:>3.0f} min  | {n.get('calories', 0):>4.0f}   | {', '.join(flags)}"
        )
    ...
    if sport_days and no_sport_days:  # Check if both list contain data
        print(
            f"• Sport Days Avg BP:   {statistics.mean(sport_days):.0f} (vs {statistics.mean(no_sport_days):.0f} no sport)"
        )
    ...
    if bad_sleep and good_sleep:  # Check if both list contain data
        print(
            f"• Bad Sleep (<6.5h) BP: {statistics.mean(bad_sleep):.0f} (vs {statistics.mean(good_sleep):.0f} good sleep)"
        )
