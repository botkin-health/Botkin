...
def load_body_composition():
    ...
    for filename in os.listdir(WEIGHTS_DIR):
        if (
            ...
            or filename == "apple_health_weights.json"
            or filename == "body_measurements.json"
        ):
            continue
        try:
...
def load_apple_health_bp():
    try:
        ...
        for entry in data.get("measurements", []):
            if not entry.get("date"):
                continue
            records.append({"date": entry["date"][:10], "systolic": entry["systolic"], "diastolic": entry["diastolic"]})
        df = pd.DataFrame(records)
        if not df.empty:
...
def load_garmin_daily():
    records = []
    if not os.path.exists(GARMIN_DAILY):
        return pd.DataFrame()
...
def load_garmin_sleep():
    records = []
    if not os.path.exists(GARMIN_SLEEP):
        return pd.DataFrame()
...
def load_garmin_stress():
    records = []
    if not os.path.exists(GARMIN_STRESS):
        return pd.DataFrame()
...
def load_garmin_hrv():
    records = []
    if not os.path.exists(GARMIN_HRV):
        return pd.DataFrame()
    for filename in os.listdir(GARMIN_HRV):
        ...
        try:
            ...
            if not date:
                continue
            records.append({"date": date, "hrv_last_night": summary.get("lastNightAvg")})
        except:
            pass
    return pd.DataFrame(records)
...
def load_garmin_bb():
    records = []
    if not os.path.exists(GARMIN_BB):
        return pd.DataFrame()
    for filename in os.listdir(GARMIN_BB):
        ...
        try:
            ...
            if not date:
                continue
            records.append({"date": date, "bb_charged": data.get("charged"), "bb_drained": data.get("drained")})
        except:
            pass
    return pd.DataFrame(records)
...
def load_netatmo():
    records = []
    if not os.path.exists(NETATMO_LOG):
        return pd.DataFrame()
    try:
...
def load_screentime():
    records = []
    if not os.path.exists(SCREENTIME_LOG):
        return pd.DataFrame()
    try:
...
def main():
    ...
    print(
        f"Загружено: Вес {len(df_weight)}, Состав {len(df_comp)}, АД {len(df_bp)}, Garmin {len(df_daily)}, Сон {len(df_sleep)} (дыхание включено), Стресс {len(df_stress)}, HRV {len(df_hrv)}, Netatmo {len(df_netatmo)}, Экраны {len(df_screentime)}"
    )
    ...
    if "visceral_fat" in df_final.columns:
        first_valid = (
            df_final["visceral_fat"].dropna().iloc[0] if not df_final["visceral_fat"].dropna().empty else "N/A"
        )
        last_valid = (
            df_final["visceral_fat"].dropna().iloc[-1] if not df_final["visceral_fat"].dropna().empty else "N/A"
        )
        summary.append(f"Висцеральный жир: {first_valid} -> {last_valid}")

    if "fat_percent" in df_final.columns:
        first_valid_fat = (
            df_final["fat_percent"].dropna().iloc[0] if not df_final["fat_percent"].dropna().empty else "N/A"
        )
        last_valid_fat = (
            df_final["fat_percent"].dropna().iloc[-1] if not df_final["fat_percent"].dropna().empty else "N/A"
        )
