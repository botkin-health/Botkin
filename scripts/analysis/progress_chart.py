...
def _is_alcohol(food_str):
    f = food_str.lower()
    if any(kw in f for kw in _ALCO_SUBSTR):
        return True
...
def _query_ssh():
    ...
    kw_sql = " OR ".join(
        ...
        + ["item->>'food' ~* '\\\\mслово\\\\M'".replace("слово", w) for w in _ALCO_WORDS]
    )
    try:
        out = _sp.check_output(
            _SSH
...
print("[nutrition] querying PostgreSQL via SSH...", file=sys.stderr)
nutrition, alcohol_days = _query_ssh()

if not nutrition:
    # SSH недоступен — пробуем локальный кэш
    if _cache_fresh(NUTRITION_CACHE):
        print("[nutrition] SSH failed, trying local cache...", file=sys.stderr)
        try:
...
def ma7(data_dict):
    ...
    for i, d in enumerate(all_dates):
        ...
        dates_out.append(d)
        vals_out.append(sum(window) / len(window))
...
# ── График ────────────────────────────────────────────────────────────────────
fig, axes = plt.subplots(3, 1, figsize=(16, 11), gridspec_kw={"height_ratios": [3, 2, 0.35]}, sharex=True)
...
ax1.plot([raw_wd[-1], GOAL_DATE], [last_w, GOAL_WEIGHT], color=c["goal"], linestyle="--", linewidth=1.8, alpha=0.85)
ax1.annotate(
    "49 лет\n75 кг",
    xy=(GOAL_DATE, GOAL_WEIGHT),
...
ax3.set_title(
    f"Калорийность · Среднее: {avg_cal:.0f} ккал/день  (NutriLogBot → PostgreSQL)", color=c["txt"], fontsize=12, pad=8
)
ax3.tick_params(colors=c["txt"])
...
ax4.set_ylim(0, 1)
ax4.tick_params(colors=c["txt"])
ax4.grid(True, alpha=0.15, color=c["grid"])
train_count = len(training_dates)
weeks = (TODAY - START).days / 7
ax4.set_title(f"{train_count} тренировок ({train_count / weeks:.1f}/нед)", color=c["txt"], fontsize=11, pad=4)
