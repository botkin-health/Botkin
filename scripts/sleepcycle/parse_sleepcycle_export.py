...
DOWNLOADS_DIRS = [Path.home() / "Загрузки", Path.home() / "Downloads"]
...
def parse_sleepcycle_csv(csv_file):
    ...
    for record in records:
        try:
            ...
            parsed_record = {
                ...
                "Body temperature deviation (degrees Celsius)": record.get(
    ...
    if all_records:
        try:
            last_start = max(
                [datetime.strptime(r["Start"], "%Y-%m-%d %H:%M:%S") for r in all_records if r.get("Start")]
            )
