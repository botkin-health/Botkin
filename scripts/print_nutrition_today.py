...
Использование:
  ...
  telegram_id по умолчанию REDACTED_ID (Ника), дата — сегодня.
Для продакшена на сервере:
  docker exec -i healthvault_bot python scripts/print_nutrition_today.py REDACTED_ID
"""
import sys
...
def main():
    user_id = int(sys.argv[1]) if len(sys.argv) > 1 else REDACTED_ID
