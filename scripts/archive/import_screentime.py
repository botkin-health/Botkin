...
def get_screen_time():
    if not os.path.exists(DB_PATH):
        ...
        print(
            "Если у вас macOS 13+, данные могут храниться в Biome. Убедитесь, что у скрипта есть 'Полный доступ к диску' (Full Disk Access)."
        )
        return []
    ...
    try:
        ...
        for row in pickup_rows:
            pickups.append({"type": "screen_pickup", "time": row[0], "device_id": row[2]})
...
def generate_summary(events, pickups):
    ...
    def categorize(bundle):
        if not bundle:
            return "Other"
        b = bundle.lower()
        if any(x in b for x in SOCIAL_APPS):
            return "Social & Entertainment"
        if any(x in b for x in MSG_APPS):
            return "Messengers"
        if any(x in b for x in WORK_APPS):
            return "Work & Web"
        return "Other"
