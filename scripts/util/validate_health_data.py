...
class HealthDataValidator:
    ...
    def _validate_export_freshness(self):
        ...
        try:
            ...
            if export_date:
                ...
                self.validation_results["export_freshness"] = {"date": export_date, "days_ago": days_ago}

                if days_ago > 7:
                    self.validation_results["critical_issues"].append(
                        f"❌ Экспорт Apple Health устарел ({days_ago} дней)"
                    )
                elif days_ago > 3:
                    self.validation_results["warnings"].append(
                        f"⚠️ Экспорт Apple Health старше 3 дней ({days_ago} дней)"
                    )
                else:
    ...
    def _validate_workout_data(self):
        ...
        try:
            ...
            if total_workouts_30_days == 0:
                ...
                self.validation_results["warnings"].append(
                    f"⚠️ Мало тренировок: {total_workouts_30_days} за 30 дней (ожидается 12+)"
                )
            else:
    ...
    def _validate_body_composition(self):
        ...
        try:
            ...
            if recent_days < 3:
                self.validation_results["warnings"].append(
                    f"⚠️ Мало измерений состава тела: {recent_days} дней за неделю"
                )
            else:
    ...
    def _validate_nutrition_data(self):
        ...
        try:
            ...
            self.validation_results["nutrition"] = {
                ...
                "avg_calories": sum(e["totals"].get("calories", 0) for e in recent_entries) / len(recent_entries)
                if recent_entries
                else 0,
            }
                    ...
                    if "Sleep" in record_type:
                        ...
                        if start_date:
                            try:
                                ...
                                if date_obj >= datetime.now(date_obj.tzinfo) - timedelta(days=7):
                                    recent_sleep.append(
                                        {"source": source, "date": date_obj.strftime("%Y-%m-%d"), "type": record_type}
                                    )
                            except:
                                continue
                elem.clear()
...
def main():
    ...
    results_file = (
        validator.project_root / "logs" / f"validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
