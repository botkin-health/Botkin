...
sshpass -p "$PASS" ssh -o StrictHostKeyChecking=no $SERVER "docker exec healthvault_postgres psql -U healthvault -d healthvault -t -c \"COPY (SELECT json_agg(t) FROM (SELECT date::text, meal_time::text, items, totals FROM nutrition_log WHERE date >= '2026-01-01' AND user_id = 895655) t) TO STDOUT\"" > data/nutrition/nutrition_log_remote.json
