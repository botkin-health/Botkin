...
- Показать черновик (title + body + label) → дождаться подтверждения → создать. Новый issue — это сырая постановка, ещё не оценённая, поэтому ставим **триаж-метку `needs-triage`** вместе с типом:
  ```bash
  gh issue create --title "<≤5 слов>" --label <label> --label needs-triage --body "$(cat <<'EOF'
  <постановка/проблема>
  EOF
  )"
  ```
...
- **Лейблы (тип)** — если пусто, проставить по типу (`bug`/`enhancement`/`documentation`).
- **Триаж (зрелость)** — гейт и есть момент оценки задачи. Выставить **ровно одну** триаж-роль (см. «Триаж-метки» в Справке) и снять `needs-triage`:
...
- **Качество описания** — пустое/тривиальное → проставить `needs-info` + **СТОП**: попросить нормальную постановку + критерии приёмки, записать их в issue (`gh issue comment`/`gh issue edit`).
...
### Триаж-метки (зрелость задачи)
Канон — [docs/agents/triage-labels.md](../../../docs/agents/triage-labels.md). Сигналят, что можно брать сразу, а что ещё обдумывается:

| Метка | Когда |
|---|---|
| `needs-triage` | сырой issue, ещё не оценён (ставится при заведении) |
| `needs-info` | не хватает данных от автора — ждём уточнений |
| `ready-for-agent` | полностью описана, потянет AFK-агент |
| `ready-for-human` | требует реализации человеком |
| `wontfix` | решили не делать |

На issue висит **ровно одна** триаж-роль. Переключение (на гейте 1.5):
```bash
# выставить роль и снять предыдущую (напр. оценили needs-triage → ready-for-agent)
gh issue edit <N> --repo botkin-health/Botkin --add-label ready-for-agent --remove-label needs-triage
```
