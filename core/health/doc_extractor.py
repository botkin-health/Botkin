...
# Только явные оценочные обороты. Одиночные «норма», «снижение», «повышен» — это часто
# напечатанный результат («Уробилиноген: норма») или рекомендация врача (ревью #560).
_VERDICT_RE = re.compile(
    r"(?<!\bне\s)в\s+пределах\s+(?:нормы|референсн)|(?<!\bне\s)в\s+норме\b|вс[её]\s+в\s+порядке|без\s+отклонений"
    r"|не\s+выход\w*\s+за\s+пределы",
    re.I,
)
...
_VERDICT_CLAUSE_RE = re.compile(
    r",?\s*(?:все\s+)?(?:показател\w*\s+)?(?:наход\w*\s+)?(?<!\bне\s)в\s+(?:пределах\s+(?:нормы|референсн\w*\s+значени\w*)|норме)",
    re.I,
)
