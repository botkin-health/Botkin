...
# Коды МКБ-10 группы Z — факторы обращения (осмотр, обследование, наблюдение),
# не заболевания. В медпрофиль как «диагноз» попадать не должны (#558).
_Z_CODE_RE = re.compile(r"\bZ\d{2}(?:\.\d+)?\b")


def _filter_conditions(data: dict) -> None:
    """Убирает из conditions пункты с кодом Z; убранное — в `_dropped_conditions`."""
    kept, dropped = [], []
