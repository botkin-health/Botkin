...
TOOLS: list[dict[str, Any]] = [
            ...
            "properties": {
                ...
                "measured_at": {
                    ...
                    "description": (
                        ...
                        "Без указания — не передавай, запишется текущим моментом. Если "
                        "пользователь назвал часть суток («утром», «вечером») — передавай "
                        "ПОЛНОЕ время (утро ~08:00, вечер ~20:00), не только дату: два "
                        "замера за один день с одной лишь датой без времени могут "
    ...
    },
    {
        ...
        "description": (
            ...
            "Если пользователь указал, когда это было («вчера», «в понедельник», время) — "
            "вычисли и передай start_time (YYYY-MM-DDTHH:MM:SS). Без указания — не передавай "
            "(запишется текущим моментом). Дату не сегодняшнего дня назови в ответе.\n\n"
...
def ask_agent(
    ...
    try:
        ...
        UNIVERSAL_META_PROMPT = (
            ...
            "\n"
            "Приём или пропуск конкретной дозы — даже сказанный про прошлое («принял\n"
            "вчера», «забыл выпить позавчера») — это СОБЫТИЕ → `log_supplement` с датой.\n"
            "Постоянная терапия и диагнозы («постоянно принимаю», «у меня гипертония») →\n"
            "`save_health_profile`. Фраза часто несёт оба смысла — тогда зови ОБА тула;\n"
            "заменять запись события записью в профиль нельзя, событие потеряется.\n"
            "При неоднозначности — переспроси, не выбирай молча.\n"
            "\n"
            "---\n"
            "\n"
