...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "В пустом ответе приходят `last_point_ever_local`, `days_since_last_point` и "
            "`all_time_count` — последняя точка за ВСЮ историю; бери историю оттуда, а не из памяти. "
    ...
    },
    {
        ...
        "description": (
            ...
            "inconclusive и т.п.), средний пульс, длительность, симптомы. days=1..365, по умолчанию 90. "
            "Для вопросов «покажи мои ЭКГ», «что показала кардиограмма», «есть ли мерцалка по часам». "
            "Это ОТДЕЛЬНЫЙ источник от клинических ЭКГ в KB — там сканы из клиники, здесь записи с часов."
        ),
        "input_schema": {
            "type": "object",
    ...
    {
        "name": "get_heart_rate_events",
        "description": (
            "Уведомления Apple Watch о пульсе вне нормы В ПОКОЕ: начало/конец, тип (высокий/низкий/"
            "нерегулярный), порог, min/max/средний пульс, длительность. days=1..365, по умолчанию 90. "
            "Для вопросов «были ли уведомления о высоком пульсе», «когда была тахикардия», "
            "а также когда надо сопоставить эпизод тахикардии с гипогликемией по времени."
        ),
        "input_schema": {
            "type": "object",
            "properties": {"days": {"type": "integer", "minimum": 1, "maximum": 365, "default": 90}},
    ...
    },
    {
        ...
        "description": (
            ...
            "У замеров с весов Withings в latest есть ещё `heart_rate` (пульс — меряется "
            "стоя натощак, годится как пульс покоя), `bmr_kcal` (ИЗМЕРЕННЫЙ основной обмен — "
            "бери его, а не считай по формуле Миффлина), `fat_mass_kg` и `lean_mass_kg` "
            "(жировая и тощая масса в КИЛОГРАММАХ, а не в %). ВАЖНО: в тот же день HAE пишет "
            "свою строку позже, и в `latest` эти поля будут пустыми — тогда бери их из блока "
            "`body_composition` (там своя дата: это последний замер С СОСТАВОМ ТЕЛА, он может "
            "быть на день-два старше `latest`; дату проговаривай). "
            "Пульс по весам — источник для "
            "вопроса «растёт ли у меня пульс»: с series=true он приходит в каждой точке как `pulse`. "
            ...
            "ежедневных замеров {date, weight_kg, body_fat_pct, pulse} в окне. "
    ...
    },
    {
        ...
        "description": (
            ...
            "постоянная терапия (лекарство, которое принимает регулярно) и/или аллергии. "
            "Зови когда пользователь сам описал ПОСТОЯННОЕ состояние или схему («у меня "
            "гипертония», «постоянно пью розувастатин») — или когда ответил на твой "
            ...
            "НЕ подменяй этим тулом log_supplement: разовый приём или пропуск конкретной "
            "дозы (даже упомянутый в прошлом — «принял вчера», «забыл выпить позавчера») "
            "это СОБЫТИЕ, а не факт медпрофиля. Если сообщение называет и то, и другое — "
            "вызови оба тула, не выбирай один вместо другого."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        "name": "log_bp",
        "description": (
            "Залогировать измерение давления. Если пользователь указал, когда мерил "
            "(«вчера», «три дня назад», «в 8 утра») — вычисли и передай `measured_at`, "
            "опираясь на сегодняшнюю дату. Без указания — не передавай (запишется "
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "measured_at": {
                    "type": "string",
                    "description": (
                        "ISO datetime или YYYY-MM-DD измерения. Передавай явно, если "
                        "пользователь указал день/время (даже относительное — «вчера»). "
    ...
    },
    {
        ...
        "description": (
            ...
            "текущим моментом). После записи назови дату в ответе, если она не сегодняшняя.\n\n"
            "Ключ записи — start_time: повторный вызов с тем же start_time ОБНОВЛЯЕТ существующую "
            "запись (вернёт updated=true), а не создаёт вторую — это нужно и для ретраев, и для "
            "уточнений («40 минут» → «нет, погоди, 45 минут» — просто вызови снова с тем же "
            "start_time и новой длительностью, старая запись перезапишется).\n\n"
            "Если на это же start_time уже есть тренировка из ДРУГОГО источника (часы/трекер, "
            "напр. HAE/Garmin) — вернётся status=conflict_other_source, запись НЕ создастся и "
            "существующая НЕ изменится. Прочитай hint: скажи пользователю, что тренировка уже "
            "учтена, либо, если это правда другое событие, переспроси точное время."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            ...
            "Если пользователь указал ДЕНЬ/ВРЕМЯ приёма («вчера», «позавчера», «в 9 "
            "утра») — вычисли и передай `date`/`time`, опираясь на сегодняшнее число. "
            "Без указания дня — не передавай (запишется на сегодня, текущим временем).\n\n"
            "Упоминание конкретного факта приёма или пропуска дозы в прошлом («выпил "
            "капотен позавчера», «забыл принять утренние таблетки несколько дней "
            "назад») — это СОБЫТИЕ, а не только повод для save_health_profile. Даже "
            "если из фразы заодно ясно, что это постоянная терапия — сначала залогируй "
            "фактический приём здесь, отдельным вызовом сохранив терапию в медпрофиль "
            "(save_health_profile). НЕ теряй событие приёма, выбрав только медпрофиль."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "date": {
                    "type": "string",
                    "description": (
                        "Дата приёма YYYY-MM-DD. Передавай явно, если пользователь указал "
                        "день (даже относительный — «вчера»). Без указания — не передавай, "
                        "запишется на сегодня."
                    ),
                },
                "time": {
                    "type": "string",
                    "description": "Время приёма HH:MM. Только если пользователь его назвал; иначе не передавать.",
                },
                "force": {
                    "type": "boolean",
...
def _call_tool(name: str, args: dict, token: str) -> str:
    ...
    try:
        ...
        elif name == "get_recent_ecg":
            r = requests.get(
                f"{TOOLS_API_BASE}/recent_ecg",
                params={"days": int(args.get("days", 90))},
                headers=headers,
                timeout=15,
            )
        elif name == "get_heart_rate_events":
            r = requests.get(
                f"{TOOLS_API_BASE}/heart_rate_events",
                params={"days": int(args.get("days", 90))},
                headers=headers,
                timeout=15,
            )
...
def ask_agent(
    ...
    try:
        ...
        UNIVERSAL_META_PROMPT = (
            ...
            "\n"
            "## Пусто в окне — НЕ рассказывай историю мониторинга по памяти\n"
            "Окно `get_recent_glucose` упирается в 168 часов (неделя). Если датчик\n"
            "молчит дольше — инструмент вернёт `total_count: 0` при любых параметрах,\n"
            "хотя в базе могут лежать тысячи точек за месяцы до этого.\n"
            "\n"
            "Получив `total_count: 0`:\n"
            "1. Возьми из ответа `last_point_ever_local` и `days_since_last_point` —\n"
            "   это факт о том, когда данные были в последний раз.\n"
            "2. Прежде чем говорить ХОТЬ ЧТО-ТО про историю («мониторинг шёл тогда-то»,\n"
            "   «после этого датчик сняли», «возобновление планировалось») — вызови\n"
            "   `get_glucose_stats(days=90)`. Он берёт до 90 дней и покажет, что было.\n"
            "3. ❌ ЗАПРЕЩЕНО излагать историю CGM из system-prompt, профиля или памяти\n"
            "   диалога. Контекст устаревает, база — нет.\n"
            "\n"
            "Прецедент 14.09.2026: сенсор молчал 7 дней, агент ответил «мониторинг шёл\n"
            "12.12.2025–13.01.2026, потом датчик сняли, возобновление планировалось\n"
            "14.05.2026» — выдумка из контекста: в базе лежало 2603 точки за лето,\n"
            "TIR 95,5%, последняя точка 07.09.2026.\n"
            "\n"
            "---\n"
            "\n"
            "# 💊 ЛЕКАРСТВА — СНАЧАЛА ФАРМАКОГЕНЕТИКА, ПОТОМ ОТВЕТ\n"
            "\n"
            "Если у пользователя есть результаты ДНК-теста, они лежат в KB под\n"
            "ключами `agent_corrections.genetics_pharmacogenetics`,\n"
            "`agent_corrections.genetics_findings` и\n"
            "`agent_corrections.genetics_excluded_diagnoses`.\n"
            "\n"
            "При ЛЮБОМ вопросе про лекарство — назначили, начать, заменить, «что\n"
            "скажешь про препарат X», побочные эффекты, доза — СНАЧАЛА вызови\n"
            "`get_kb_value('agent_corrections.genetics_pharmacogenetics')` и только\n"
            "потом отвечай. Если препарат есть в списке — назови вывод генетики\n"
            "ПЕРВЫМ пунктом, вместе с геном-основанием.\n"
            "\n"
            "❌ ЗАПРЕЩЕНО обсуждать статины, антикоагулянты и антиагреганты,\n"
            "антидепрессанты, опиоиды, наркоз и сахароснижающие, не заглянув туда.\n"
            "Это ровно те классы, где у человека может быть вариант, меняющий выбор\n"
            "препарата или дозу.\n"
            "\n"
            "Прецедент 20.09.2026: на вопрос «хотят назначить статин» агент разобрал\n"
            "липидограмму и печёночные ферменты, но не сказал главного — что по\n"
            "SLCO1B1 у пользователя повышен риск статиновой миопатии, симвастатин\n"
            "токсичен, аторвастатин малоэффективен, а препарат выбора —\n"
            "розувастатин. Данные лежали в KB и не были прочитаны.\n"
            "\n"
            "Чего в этих данных НЕТ — того не выдумывай: если препарата в списке\n"
            "нет, так и скажи, что по нему генетических данных не получено.\n"
            "\n"
            "## Генетика нужна не только для лекарств\n"
            "`agent_corrections.genetics_findings` — риски заболеваний с цифрами,\n"
            "варианты в генах, дефициты витаминов, липиды, непереносимости.\n"
            "`agent_corrections.genetics_excluded_diagnoses` — что ИСКЛЮЧЕНО и какие\n"
            "версии больше не рассматривать.\n"
            "\n"
            "Читай их, когда речь про:\n"
            "- риск конкретного заболевания («какой у меня риск диабета»);\n"
            "- билирубин и печёночные ферменты (носительство синдромов влияет на\n"
            "  интерпретацию);\n"
            "- дефициты витаминов и назначение добавок;\n"
            "- вес, аппетит, чувство насыщения;\n"
            "- аллергию и пищевые непереносимости;\n"
            "- тромбозы, эмболию, наследственную предрасположенность к ним;\n"
            "- обмен железа, ферритин, необъяснимые отклонения печёночных проб;\n"
            "- предстоящий наркоз или операцию.\n"
            "\n"
            "❌ Не повторяй версию, которая уже исключена генетически. Если по теме\n"
            "есть запись в `genetics_excluded_diagnoses` — сначала скажи, что эта\n"
            "причина снята, и только потом обсуждай оставшиеся.\n"
            "\n"
            "Прецедент 20.09.2026: на вопрос «какой у меня риск диабета и что с\n"
            "билирубином» агент ответил только по старым анализам — не назвал\n"
            "полигенный риск диабета 2 типа 11 % против 3 % популяционных и не\n"
            "сказал про носительство синдрома Жильбера (UGT1A1 rs887829 CT), которое\n"
            "как раз и меняет трактовку билирубина. Обе записи лежали в KB.\n"
            "\n"
            "---\n"
            "\n"
            "# 🫀 ЭКГ И СОБЫТИЯ ПУЛЬСА — ОТДЕЛЬНЫЕ ИНСТРУМЕНТЫ\n"
            "\n"
            "`get_recent_ecg` — записи ЭКГ с Apple Watch (заключение, средний пульс).\n"
            "`get_heart_rate_events` — уведомления часов о пульсе вне нормы в покое.\n"
            "\n"
            "Вызывай их при любом вопросе про кардиограмму, тахикардию, «высокий пульс»,\n"
            "мерцательную аритмию по часам. НЕ подменяй их KB-агрегатами: в KB лежат\n"
            "клинические ЭКГ из клиники и старые сводки, а свежие записи с часов — только\n"
            "в этих инструментах. Эпизод тахикардии имеет смысл сопоставлять по времени\n"
            "с глюкозой: 20.08.2026 уведомление о пульсе >100 совпало со снижением до 3,10.\n"
            "\n"
            "Прецедент 15.09.2026: на вопрос «покажи мои ЭКГ» агент ответил, что\n"
            "поэлементных записей у него нет, и выдал KB-сводку годичной давности; на\n"
            "вопрос про уведомления о пульсе — «посмотри сам в приложении Health».\n"
            "Инструментов тогда не было, теперь есть.\n"
            "\n"
            "---\n"
            "\n"
            ...
            "---\n"
            "\n"
            "# 💊 vs 📝 РАЗВИЛКА: приём или медпрофиль (универсальный)\n"
            "\n"
            "Упоминание лекарства — это одно из двух, не путай:\n"
            "- Разовое СОБЫТИЕ приёма, даже сказанное в прошлом («принял вчера», «забыл\n"
            "  выпить позавчера», «пропустил утреннюю таблетку несколько дней назад») →\n"
            "  `log_supplement` с датой.\n"
            "- Факт о ПОСТОЯННОЙ терапии/диагнозе («постоянно принимаю», «пью на\n"
            "  регулярной основе», «у меня гипертония») → `save_health_profile`.\n"
            "\n"
            "Фраза часто несёт оба смысла сразу («выпил капотен, я его постоянно\n"
            "принимаю») — тогда зови ОБА тула. При неоднозначности — переспроси, не\n"
            "выбирай молча. Упоминание конкретного приёма или пропуска дозы ОБЯЗАТЕЛЬНО\n"
            "идёт в log_supplement, даже если заодно пишешь в медпрофиль — нельзя\n"
            "заменять запись события записью в профиль.\n"
            "\n"
            "Прецедент #507: «капотен и пропущенные утренние таблетки были несколько\n"
            "дней назад» — агент вызвал только save_health_profile, событие приёма\n"
            "потерялось целиком, а пользователь решил, что приём записан.\n"
            "\n"
            "---\n"
            "\n"
        ...
        _date_line = (
            ...
            "Это касается ЛЮБОГО инструмента записи данных (давление, добавки, "
            "тренировки, еда и т.д.): если пользователь упомянул прошлое — либо "
            "вычисли и передай дату/время явно, либо переспроси; молчаливая запись "
            "«на сейчас» при явном упоминании прошлого недопустима.\n\n"
        )
