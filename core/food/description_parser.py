...
def extract_products_from_description(description: str) -> List[Dict[str, any]]:
    ...
    weight_patterns = [
        ...
        r'(\d+(?:[.,]\d+)?)\s*(?:грамм|граммов|г|g|Г|G|мл|миллилитр[а-я]*|л|литр[а-я]*)[ \t]+([а-яёa-zA-Z-]+(?:\s+[а-яёa-zA-Z-]+){0,2})',
        
        # 2. "сыр 30г" или "кола 200 мл" - support full 'грамм' word
        # Исключаем слова, связанные с весом, из названия продукта
        r'([а-яёa-zA-Z-]+(?:\s+[а-яёa-zA-Z-]+){0,2})\s+(\d+(?:[.,]\d+)?)\s*(?:грамм|граммов|г|g|Г|G|мл|миллилитр[а-я]*|л|литр[а-я]*)\b',
    ]
    ...
    for weight_pattern in weight_patterns:
        for match in re.finditer(weight_pattern, description_lower[search_start:]):
            ...
            if len(match.groups()) == 2:
                ...
                product_name = re.sub(r'^(грамм|граммов|г|g|мл|миллилитр[а-я]*|л|литр[а-я]*)\s+', '', product_name, flags=re.IGNORECASE).strip()
                product_name = re.sub(r'\s+(грамм|граммов|г|g|мл|миллилитр[а-я]*|л|литр[а-я]*)$', '', product_name, flags=re.IGNORECASE).strip()
    ...
    def replace_text_numbers(text):
        text_numbers = {
            ...
            'девять': '9',
            'десять': '10',
            'пол': '0.5', 'половину': '0.5', 'половина': '0.5', 'половинку': '0.5',
        }
    ...
    quantity_patterns = [
        ...
        (_N + r'\s*(?:[а-яё]+\s+)?(?:перц[а-я]*|перец)', 150, 'перец фаршированный' if 'тушон' in description_lower or 'фарш' in description_lower else 'перец'),
        (_N + r'\s*(?:[а-яё]+\s+)?(?:голубц[а-я]*|голубец)', 200, 'голубцы'),
        (_N + r'\s*(?:[а-яё]+\s+)?(?:котлет[а-я]*)', 80, 'котлета'),
        (_N + r'\s*(?:[а-яё]+\s+)?(?:сосиск[а-я]*)', 50, 'сосиска'),
        (_N + r'\s*(?:[а-яё]+\s+)?(?:сырник[а-яё]*)', 50, 'сырник'),
        (_N + r'\s*(?:[а-яё]+\s+)?(?:оладь[а-яё]*)', 40, 'оладья'),
        
        # Bombbar протеиновые батончики
        (_N + r'\s*(?:протеинов[а-я]*\s+)?батончик[а-яё]*\s+(?:bombbar|бомббар|бомбар|bomb\s*bar)', 40, 'батончик bombbar'),
        (_N + r'\s*(?:bombbar|бомббар|бомбар|bomb\s*bar)\s+(?:протеинов[а-я]*\s+)?батончик[а-яё]*', 40, 'батончик bombbar'),
    ]
