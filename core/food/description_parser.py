...
# в core/food/nutrition.py.
SUBPORTION_RE = re.compile(
    r"(ложк|ложек|ложечк|\bст\.?\s?л\.?|\bч\.?\s?л\.?|кусоч|\bкусок\b|\bкуски\b|ломтик|"
    r"горст|щепот|половин|\bполовину\b|четверт|\bтрет[ьи]\b|глоток|глотк|"
...
def extract_products_from_description(description: str) -> List[Dict[str, any]]:
    ...
    weight_patterns = [
        ...
        # Баг #449: без (?!\s*(?:и|или)\s) паттерн пересекал границу продуктов —
        # "рис 180 г и лосось 90 г" матчился целиком как "180 г и лосось" (вес риса
        # приписывался фантомной записи "и лосось"). Негативный lookahead запрещает
        # захватывать название, начинающееся с союза-разделителя продуктов.
        # Баг #449 follow-up (review нашёл половину фикса): тот же союз мог попасть
        # СЕРЕДИНОЙ 3-словного захвата имени — "180 г отварного риса и лосось"
        # матчился как имя "отварного риса и лосось" целиком (расширение {0,2}
        # проглатывало "и" как одно из двух дополнительных слов). Добавлен
        # lookahead на КАЖДОЕ слово расширения: (?!(?:и|или)\b) перед [ \t]+
        # запрещает следующему слову расширения быть союзом-разделителем,
        # поэтому расширение обрывается на границе продукта, а не поглощает её.
        # Note: (?!\s*(?:и|или)\s) ниже использует \s (матчит и \n), а не [ \t] —
        # это НЕ нарушает историческую гарантию "только горизонтальный пробел"
        # (см. коммит 712600d, инцидент с "73 г перца \nСтоловая ложка масла"):
        # lookahead нулевой ширины, сам ничего не захватывает, а если и матчит
        # перенос строки — просто блокирует матч паттерна в этой позиции. Реальный
        # захват имени всё равно ограничен [ \t]+ снаружи, так что через \n
        # ничего захватить физически нельзя ни с \s, ни с [ \t] в lookahead.
        r"(\d+(?:[.,]\d+)?)\s*(?:грамм|граммов|г|g|Г|G|мл|миллилитр[а-я]*|л|литр[а-я]*)(?!\s*(?:и|или)\s)[ \t]+([а-яёa-zA-Z-]+(?:[ \t]+(?!(?:и|или)\b)[а-яёa-zA-Z-]+){0,2})",
        ...
        # Баг #449 follow-up: тот же союз-в-середине баг у паттерна name-first —
        # "риса или лосось 90 г" матчился как единое имя "риса или лосось" (весь
        # 90г относился к фантомному "риса или лосось" вместо отдельного "лосось",
        # и после этого дублировался с паттерном 1 и терялся при dedup). Тот же
        # per-word lookahead в расширении не даёт "или"/"и" стать вторым/третьим
        # словом захвата. Слово "и"/"или" всё ещё может начать НОВЫЙ отдельный
        # матч (например "или лосось 90 г") — это ловится отдельной чисткой
        # ведущего союза ниже (re.sub(r"^(?:и|или)\s+", ...)), так что итоговое
        # имя всё равно очищается до "лосось".
        r"([а-яёa-zA-Z-]+(?:[ \t]+(?!(?:и|или)\b)[а-яёa-zA-Z-]+){0,2})[ \t]+(\d+(?:[.,]\d+)?)\s*(?:грамм|граммов|г|g|Г|G|мл|миллилитр[а-я]*|л|литр[а-я]*)\b",
    ]
    ...
    for weight_pattern in weight_patterns:
        for match in re.finditer(weight_pattern, description_lower[search_start:]):
            ...
            if len(match.groups()) == 2:
                ...
                # Баг #449: было r"\s*(и|или|с|из|для|на|в|:)\s*$" — без \b/пробела перед
                # буквенными предлогами альтернатива "с" матчила последнюю букву слова
                # "рис" → "ри" (аналогично любое слово, оканчивающееся на и/в/с и т.д.).
                # Двоеточие оставляем на \s* (это отдельный знак пунктуации, а не буква,
                # с которой могло бы слипнуться слово), а предлоги-слова требуют
                # словной границы \b перед собой.
                product_name = re.sub(r"\b(?:и|или|с|из|для|на|в)\s*$|\s*:\s*$", "", product_name).strip()
                product_name = re.sub(r"^[:\s]+", "", product_name).strip()  # Убираем двоеточие в начале
                # Баг #449: паттерн 1 (число-первым) с lookahead выше больше не захватывает
                # ведущий союз, но паттерн 2 (название-первым) всё ещё может дать
                # "и лосось"/"или лосось" как name — убираем ведущий союз отдельно,
                # чтобы в списке продуктов не появлялись фантомные "и ..." записи.
                # flags=re.IGNORECASE здесь technically dead code: product_name всегда
                # получено из уже нижнерегистрового description_lower. Оставлено как
                # defensive/harmless на случай если источник product_name когда-нибудь
                # перестанет быть гарантированно lower().
                product_name = re.sub(r"^(?:и|или)\s+", "", product_name, flags=re.IGNORECASE).strip()
