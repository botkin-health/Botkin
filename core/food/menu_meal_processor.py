...
def process_meal_description_with_menu(
    ...
    logger.info(f"КБЖУ из меню (на 100г): {menu_nutrition['calories']} ккал, "
...
def _is_menu_product(product: Dict, product_name: str, menu_dish_name: str, description: str) -> bool:
    ...
    stop_words = {'в', 'на', 'с', 'из', 'для', 'и', 'или', 'как', 'что', 'это',
                  'the', 'a', 'an', 'in', 'on', 'at', 'for', 'with',
                  'калининградский', 'кусок', 'соус', 'унаги', 'стерилизованные', 'консервы'}
    ...
    if (is_fish or
        ...
        any(kw in product_name for kw in menu_keywords if len(kw) > 3) or
        any(kw in menu_dish_name for kw in product_keywords if len(kw) > 3)):
        return True
...
def _process_menu_product(product: Dict, product_weight, menu_data: Dict,
                          menu_nutrition: Dict, menu_weight) -> Dict:
...
def _process_regular_product(product: Dict, description: str) -> Optional[Dict]:
    ...
    if product.get('source') == 'description_simple':
        default_weights = {
            'кофе': 250, 'чай': 250, 'сок': 200, 'вода': 250, 'молоко': 250,
        }
