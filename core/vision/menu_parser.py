...
def extract_nutrition_from_text(text: str) -> Optional[Dict[str, float]]:
    ...
    all_labels = re.findall(
        r"\b(protein|белка|белок|proteins|fat|жиров|жир|fats|carbs|carbohydrates|углеводов|углеводы)\b",
        text,
        re.IGNORECASE,
    )
            ...
            if best_match:
                ...
                if "protein" in label_text or "белк" in label_text:
                    ...
                    if "carbs" not in nutrition:
                        ...
                        used_numbers.add(num_pos)
                        print(
                            f"    ✅ Углеводы сопоставлены: {num_value}г (метка '{label_text}' на позиции {label_pos})"
                        )
    ...
    if nutrition:
        ...
        if fats > 80:
            ...
            print("    💡 Возможно, OCR неправильно распознал значение. Проверьте исходный текст.")
        if carbs > 80:
...
def parse_menu_photo(
    photo_paths: Union[Path, List[Path]],
    ...
    if use_chatgpt and CHATGPT_AVAILABLE and parse_menu_with_chatgpt:
        ...
        if openai_key:
            ...
            if result:
                return result
            print("    ⚠️  ChatGPT не распознал, пробую Gemini...")
    ...
    try:
        ...
        dish_name = extract_dish_name_from_text(text) or "Неизвестный продукт"

        if not nutrition:
            print("    ❌ КБЖУ не найдено в тексте меню")
            # CHANGED: Return text for product search fallback
            return {
                "dish_name": dish_name,
                "raw_text": text,
        ...
        if weight_grams and weight_grams > 0:
            ...
            if re.search(r"(?:на|в|/)\s*100\s*(?:г|g|грам)", text, re.IGNORECASE):
                ...
                print(
                    f"       Калории: {nutrition_per_100g['calories']} ккал/100г × {multiplier:.2f} = {nutrition['calories']:.1f} ккал"
                )
                print(
                    f"       Белки: {nutrition_per_100g['protein']}г/100г × {multiplier:.2f} = {nutrition['protein']:.1f}г"
                )
                print(f"       Жиры: {nutrition_per_100g['fats']}г/100г × {multiplier:.2f} = {nutrition['fats']:.1f}г")
                print(
                    f"       Углеводы: {nutrition_per_100g['carbs']}г/100г × {multiplier:.2f} = {nutrition['carbs']:.1f}г"
                )
            else:
        ...
        # Извлекаем название блюда (если не извлекли ранее)
        if not dish_name or dish_name == "Неизвестный продукт":
        ...
        return {
            ...
            "source": "menu_ocr",
            "raw_text": text,  # Also return text just in case
        }
