...
def extract_weight_from_text(text: str) -> Optional[float]:
    ...
    for space_match in space_matches:
        ...
        try:
            ...
            if 1 <= weight_test <= 5000:
                ...
                text_corrected = text_corrected[:space_match.start()] + \
                               re.sub(space_number_pattern, f'000 {combined}', text_corrected[space_match.start():space_match.end()], count=1) + \
    ...
    print(f"    ❌ Вес не найден в тексте")
    return None
...
def extract_weight_from_photo_with_context(photo_path: Path, api_key: Optional[str] = None, previous_weights: List[float] = None) -> Optional[float]:
    """
    ...
    try:
        ...
        if 163.9 in previous_weights and '16 39' in text and '000' in text:
            print(f"    🔧 Контекстная проверка: уже есть 163.9г, '16 39' может быть ошибкой OCR для '14 53'")
            # Пробуем исправить "16 39" на "14 53" в тексте
            text_corrected = text.replace('16 39', '14 53')
            print(f"    🔧 Попытка исправления: '16 39' -> '14 53'")
            ...
            if weight and weight == 145.3:
                ...
                print(f"    ⚠️  Контекстное исправление не сработало, используем оригинальный текст")
