...
СЦЕНАРИЙ 2: РЕЦЕПТУРНАЯ КАРТОЧКА (Elementaree, meal kit и т.п.)
Если на фото БУМАЖНАЯ КАРТОЧКА РЕЦЕПТА с напечатанными КБЖУ:
- Признаки: слова "РЕЦЕПТ", "Блюдо №", КБЖУ в шапке (напр. "498 ккал | Б 43 г | Ж 20 г | У 36 г"), вес порции, количество порций
- ЧИТАЙ КБЖУ ПРЯМО С КАРТОЧКИ. НЕ угадывай и НЕ пересчитывай!
- Определи: на сколько порций рецепт (обычно 2)
- Верни КБЖУ ЗА ОДНУ ПОРЦИЮ (раздели на число порций если указано на всё блюдо)
- Формат JSON:
{{
  "is_recipe_card": true,
  "dish_name": "название блюда с карточки",
  "calories": число (НА 1 ПОРЦИЮ),
  "protein": число (НА 1 ПОРЦИЮ),
  "fats": число (НА 1 ПОРЦИЮ),
  "carbs": число (НА 1 ПОРЦИЮ),
  "weight_grams": число (вес 1 порции),
  "servings": число (на сколько порций рецепт),
  "source": "recipe_card"
}}

СЦЕНАРИЙ 3: ЕДА / ПРОДУКТЫ / МЕНЮ
...
- ДЛЯ УПАКОВОК: Обязательно прочти "Масса нетто" или "Net weight" с упаковки — это и есть weight_grams. Рассчитай КБЖУ для этого веса (данные с этикетки × вес / 100).
